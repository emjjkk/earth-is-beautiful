<script>
  ...
  async function getRandomPhoto() {
    const categories = [...locationCategories].sort(() => Math.random() - 0.5);
    for (const category of categories.slice(0, 2)) {
      try {
        ...
        return {
          ...
          artist: stripHtml(metadata.Artist?.value) || "Unknown photographer",
          licenseName: metadata.LicenseShortName?.value || "",
          lat: coordinate.lat,
          lon: coordinate.lon,
        };
      } catch {}
    }
    throw new Error("No suitable landscape photos came back. Try again.");
  }
  ...
  async function reverseGeocode(latitude, longitude, id) {
    try {
      ...
      placeName = [locality, country].filter(Boolean).join(", ");
      if (!placeName) {
        placeName = data.display_name || "Somewhere on Earth";
      }
    } catch {
      if (id === requestId) {
...
<main
  ...
  style={status === "ready" ? `background-image:url("${imageUrl}")` : ""}
>
  {#if status === "loading"}<div class="state-message">
      <span class="dot"></span>Finding somewhere on Earth&hellip;
    </div>{:else if status === "error"}<div class="state-message">
      <span>{errorMessage}</span><button
        class="retry"
        onclick={fetchRandomPhoto}>Try another</button
      >
    </div>{/if}{#if status === "ready"}<a
      class="info"
      href={pageUrl}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={`Open photo source on Wikimedia Commons: ${placeName}`}
      ><span class="place">{placeName || "Locating…"}</span><span
        class="separator">·</span
      ><span>Photo by {artist}</span></a
    >{/if}
</main>
